...
func GetMostRecentUnusedSuggestions(db *gorm.DB, guildID string) []Suggestion {
	var suggestions []Suggestion
	// Get unused suggestions that have enough upvotes, ordered by most recent first
	minUpdicks := config.MinUpdicksToQualify(db, guildID)
	db.Where("used = ? AND updicks >= ?", false, minUpdicks).
		Order("created_at DESC").
		Find(&suggestions)
	logger.Debug("Found unused suggestions", "count", len(suggestions), "min_updicks", minUpdicks)
	return suggestions
}
